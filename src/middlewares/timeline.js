...
export const GetAllTimelineUsersOwner = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        ...
        const response = await axios.get(`https://gold-cloudy-moose.cyclic.app/api/v1/owner/getCompanyemployee`, {
            headers: body,
        })
        if (response.status) {
...
export const GetTimelineUsersAdmin = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://gold-cloudy-moose.cyclic.app/api/v1/superAdmin/allEmployeesworkinghour`, {
            headers: body,
        })
        if (response.status) {
            dispatch(setLoading(false))
...
export const GetTimelineUserOwner = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://gold-cloudy-moose.cyclic.app/api/v1/owner/sorted-datebased/${userId}?date=${encodeURIComponent(formattedDate)}`, {
            headers: headers,
        })
        if (response.status) {
            dispatch(setLoading(false))
...
export const GetTimelineUserSuperAdmin = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://gold-cloudy-moose.cyclic.app/api/v1/superAdmin/sorted-datebased/${userId}?date=${encodeURIComponent(formattedDate)}`, {
            headers: headers,
        })
        if (response.status) {
            dispatch(setLoading(false))
...
export const GetOwnerTeam = createAsyncThunk("get/team-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://gold-cloudy-moose.cyclic.app/api/v1/owner/companies`, {
            headers: headers,
        })
        if (response.status) {
